function add_todo(){
 ...
 node.className = "list-group-item list-group-item-primary mb-2" //mb=margain bottom
 const textnode = document.createTextNode(x)
 node.appendChild(textnode)
 var b = document.createElement('button') // b is assign to new button (createElement is used)
 b.className = "btn btn-danger"
 b.textContent = "Delete"
 b.style= "float:right;"
 b.onclick = delete_todo
 node.appendChild(b)
 var e = document.createElement('button') // c is assign to new button (createElement is used)
 e.className = "btn btn-warning"
 e.textContent = "Edit"
 e.style= "float:right;"
 e.onclick = edit_todo
 node.append(e) //button is append to node as a child
 document.getElementById('result').appendChild(node)
 document.getElementById('task').value=''  // placeholder become empty when todo button is clicked 
}
...
function delete_todo(e){
    e.target.parentElement.remove() //parent element node is deleted

}
function edit_todo(e){
    const newdiv = document.createElement('div')
    newdiv.className = "contanier text-center mt-3"
